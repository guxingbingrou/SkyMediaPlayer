...
JNIEnv* GetEnv() {
  ...
  RTC_CHECK((((env != nullptr) && (status == JNI_OK)) ||
      ...
      ,"Unexpected GetEnv return:%d:%d " ,status ,env);
...
static void ThreadDestructor(void* prev_jni_ptr) {
  ...
  RTC_CHECK((GetEnv() == prev_jni_ptr)
      ,"Detaching from another thread:%x:%x ",prev_jni_ptr, GetEnv());
  jint status = g_jvm->DetachCurrentThread();
  RTC_CHECK((status == JNI_OK) ,"Failed to detach thread:%d ", status);
  RTC_CHECK(!GetEnv(), "Detaching was a successful no-op???");
}

static void CreateJNIPtrKey() {
  RTC_CHECK(!pthread_key_create(&g_jni_ptr, &ThreadDestructor), "pthread_key_create");
}
...
static std::string GetThreadId() {
  ...
  RTC_CHECK(snprintf(buf, sizeof(buf), "%ld",
                        static_cast<long>(syscall(__NR_gettid)))< sizeof(buf),
